...
const nonTrigs = ["log", "log10", "log1p", "log2", "abs", "cbrt", "ceil", "exp", "expm1", "floor", "round", "sign", "sqrt", "trunc"];
...
nonTrigs.forEach(name => {
    methods[name] = x => {
        const result = {value: Math[name](x)};
        result.warnings = !isGood(result.value) ? [`${name}(${x}) = ${result.value}`] : [];
        return result;
    }
});

trigs.forEach(([name, invName]) => {
    ["", "h"].forEach(suffix => {
